// utils/animatedProfile.js – Simplified (static GIF background, ready for animation later)
...
async function generateAnimatedProfile(gifUrl, data) {
  ...
  // Load the GIF background as a single image (first frame)
  let bgImg;
  try {
    bgImg = await loadImage(gifUrl);
  } catch (err) {
    console.error("Failed to load GIF background:", err);
    throw err;
  }
  ...
  // Draw profile overlay on a single canvas
  ...
  // Return a static PNG buffer – this will always work
  console.log(`✅ Animated profile (static fallback) generated`);
