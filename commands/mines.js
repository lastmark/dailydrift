// commands/mines.js – Mines game (pick tiles, avoid the bomb, cash out)
const {
  SlashCommandBuilder,
  EmbedBuilder,
  ActionRowBuilder,
...
// Multiplier after each safe pick (index 0 = after 1st pick, 1 = after 2nd, etc.)
...
module.exports = {
  ...
  async execute(interaction, client, redis) {
    ...
    const gameState = {
      ...
      status: "playing",  // "playing", "cashed_out", "bust"
    };
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        ...
        `Pick a tile (1‑9) to reveal. One is a **bomb**!\n` +
        `Current multiplier: **1.00×**`
      )
      .setFooter({ text: "Choose wisely…" });

    // Build 9 tile buttons + cash out button
    const tileButtons = [];
    ...
    // Split into rows (3 rows of 3 + cashout row)
    const rows = [];
    for (let r = 0; r < 3; r++) {
    ...
    collector.on("collect", async (btnInteraction) => {
      ...
      if (btnInteraction.customId === "mines_cashout") {
        ...
        const multiplier = MULTIPLIERS[numPicked - 1];  // e.g., 1st pick = 1.5x
        ...
        await btnInteraction.update({ embeds: [embed], components: disabledRows });
      ...
      if (tileNum === state.minePos) {
        ...
        // Disable all buttons, highlight the bomb
        const disabledRows = rows.map(row =>
          new ActionRowBuilder().addComponents(
            row.components.map(btn => {
              ...
              if (btnNum === tileNum) {
                b.setLabel("💣").setStyle(ButtonStyle.Danger);
              } else if (btn.data.custom_id === "mines_cashout") {
      ...
      const numPicked = state.picked.length;
      const currentMult = numPicked > 0 ? MULTIPLIERS[numPicked - 1] : 1.0;
      const maxMult = MULTIPLIERS[MULTIPLIERS.length - 1];  // 10x

      // Update buttons: disable picked tiles, show ✓
        ...
        new ActionRowBuilder().addComponents(
          row.components.map(btn => {
            ...
            if (state.picked.includes(btnNum)) {
              b.setLabel("✓").setStyle(ButtonStyle.Secondary).setDisabled(true);
            }
            return b;
          })
        )
      ...
      const embed = EmbedBuilder.from(message.embeds[0])
        ...
        .setDescription(
          ...
          `Max possible: **${maxMult}×** (pick all 8 safe tiles)\n\n` +
    ...
    collector.on("end", async () => {
      // Timeout – if still playing, auto‑cashout with current multiplier? Or lose? We'll auto‑cashout for fairness.
      const raw = await redis.get(`mines:${userId}`);
      if (!raw) return;
      const state = JSON.parse(raw);
      if (state.status === "playing") {
        const numPicked = state.picked.length;
        if (numPicked === 0) {
          // No picks, refund bet? Better to just lose the bet. We'll treat as bust.
          state.status = "bust";
          await redis.set(`mines:${userId}`, JSON.stringify(state));
          try {
            await message.edit({ content: "⏰ Game expired – you lost your bet.", embeds: [], components: [] });
          } catch (e) {}
        } else {
